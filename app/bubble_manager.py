...
class bubble_manager():
    def __init__(self, bounding_box, transcript):
        ...
        self.text_height = 0
        self.current_phrase_transcript = []
    ...
    def update(self, face_points, face_box, frame_no, frame, cap):
        ...
        current_word_info = self.get_current_word(timestamp, self.current_phrase_transcript)
        
        return self.draw_bubble(frame, self.bubble_text, current_word_info, self.bubble_rect)


    def get_current_word(self, timestamp, current_phrase_transcript):
        current_word_idx = -1
        current_word = ''
        for word in current_phrase_transcript:
            current_word_idx += 1
            if timestamp > word['start'] and timestamp < word['end']:
                current_word = word['word']
                return current_word, current_word_idx
    ...
    def draw_bubble(self, frame, bubble_text, current_word_info, bubble_rect):
        ...
        current_word, current_word_idx = current_word_info
        current_word = current_word[1:] # it starts with a space for some reason

        font, font_scale, font_color, font_thickness = self.font_info
        words_already_printed = 0
        ...
        for idx, text in enumerate(bubble_text):
            ...
            current_text_length = len(split_text)
            if words_already_printed + current_text_length >= current_word_idx + 1 and not word_highlighted:
                word_highlighted = True
                words_into_text = current_word_idx - words_already_printed

                string_to_word = ''
                for jdx in range (words_into_text):
                    string_to_word = f'{string_to_word} {split_text[jdx]}'
                
                string_to_word = f'{string_to_word[1:]} '
                if words_into_text == 0:
                    string_to_word = f''
                    
                word_x_offset = cv2.getTextSize(string_to_word, font, font_scale, font_thickness)[0][0] - 2 # small adjustment for the bold offsetting

                
                cv2.putText(frame, current_word, (x_0 + word_x_offset, self.text_height + y_0 + (text_offset * idx)), font, font_scale, (0, 0, 255), font_thickness*2)
            else:
                words_already_printed += current_text_length
    ...
    def reset_current_phrase(self, timestamp, bubble_rect):
        ...
        self.current_phrase_start = -1
        self.current_phrase_end = -1
        self.current_phrase, self.current_phrase_transcript = self.get_current_phrase(timestamp, self.transcript, bubble_rect)
    ...
    def get_current_phrase(self, timestamp, transcription, bubble_rect):
            ...
            for segment in transcription['segments']:
                for word in segment['words']:
                    if timestamp > word['start'] and timestamp < word['end']:
                        ...
                        curr_phase_transcript.append(word)
            ...
            return curr_phrase, curr_phase_transcript
